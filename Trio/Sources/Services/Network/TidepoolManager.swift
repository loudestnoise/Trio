...
extension BaseTidepoolManager {
    ...
    /// Runs inside the serializer. Pending carbs are fetched at run time, not enqueue time:
    /// that's what makes coalescing safe, and it keeps a follow-up run from re-uploading rows
    /// an earlier run already marked as uploaded.
    private func performCarbsUpload(generation: Int) async {
        ...
        debug(.service, "Tidepool: uploading carbs (\(carbs.count) pending)")
...
extension BaseTidepoolManager {
    ...
    /// Runs inside the serializer. Pending pump events are fetched and the payloads built at run
    /// time, not enqueue time: that's what makes coalescing safe, and it keeps a follow-up run
    /// from re-uploading events an earlier run already marked as uploaded.
    private func performDoseUpload(generation: Int) async {
        ...
        debug(.service, "Tidepool: uploading doses (\(events.count) pending pump events)")
...
extension BaseTidepoolManager {
    ...
    /// Runs inside the serializer. Pending samples (CGM and manual) are fetched at run time,
    /// not enqueue time: that's what makes coalescing safe, and it keeps a follow-up run from
    /// re-uploading samples an earlier run already marked as uploaded.
    private func performGlucoseUpload(generation: Int) async {
        ...
        debug(.service, "Tidepool: uploading glucose (\(glucose.count) pending)")
...
extension BaseTidepoolManager {
    ...
    /// Runs inside the serializer. The payload is built at run time, not enqueue time, so a
    /// coalesced request still uploads the latest settings state.
