...
///
/// A pipeline may only opt into coalescing (`coalesces == true`) when its operation gathers
/// everything still pending at run time (re-fetching unuploaded rows from Core Data, or
/// rebuilding the payload from current state). Dropping a coalesced request is then safe:
/// the run already waiting to start picks up the same data when it begins. The delete
/// pipelines carry a unique payload per request and must never coalesce.
...
/// Runs enqueued upload operations strictly one at a time, in order: each operation starts only
/// after the previous one has fully completed, including its network round-trip.
///
/// Serialization is global across pipelines, not per pipeline: concurrent uploads — even to
/// different endpoints — can each trigger a Tidepool session refresh that reuses the single-use
/// refresh token, corrupting the session (#1235).
///
/// Requests for a coalescing pipeline (see `TidepoolUploadPipeline.coalesces`) fold together:
/// while a run for that pipeline is enqueued but not yet started, further requests are dropped,
/// because the waiting run will fetch whatever they would have uploaded. A request arriving while
/// the pipeline's run is already executing enqueues exactly one follow-up — the executing run's
/// fetch may predate the new data — and that follow-up doubles as a retry after a failed run,
/// since failed rows are still unmarked when it re-fetches. This bounds the chain: a burst of
/// triggers costs at most one waiting run per pipeline.
///
